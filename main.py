...
if __name__ == "__main__":
    img_rgb = cv2.imread('1.png')  # 需要检测的图片
    img_resize = cv2.resize(img_rgb, (224, 300))
    img_gray = cv2.cvtColor(img_resize, cv2.COLOR_BGR2GRAY)  # 转化成灰色
    # cv2.imshow("result.jpg", img_gray)
    # cv2.waitKey()
    template_img = cv2.imread('999.jpg', 0)  # 模板小图
    template_threshold = 0.05  # 模板置信度
    ...
    cv2.imshow("result.jpg", img_resize)
    cv2.waitKey()
    cv2.imwrite("result.jpg", img_resize)
